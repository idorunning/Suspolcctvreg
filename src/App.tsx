          ...
          const unsubUser = onSnapshot(userRef, async (userSnap) => {
            try {
              if (userSnap.exists()) {
                ...
                if ((currentUser.email === 'nathan.tracey@sussex.police.uk' || currentUser.email === 'ntracey@gmail.com') && 
        ...
        <header className="bg-white h-16 border-b border-gray-200 flex items-center justify-between px-4 sm:px-6 shadow-sm z-10">
          ...
          <div className="flex items-center gap-2 sm:gap-4 ml-2 flex-shrink-0">
            ...
            <button 
              ...
              className="p-2 text-gray-500 hover:text-red-650 hover:bg-red-50 rounded-full transition-colors"
              title="Sign Out"
              type="button"
            >
              <LogOut size={20} />
